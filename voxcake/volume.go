...
func (volume *volumeStruct) Load(file string) {
	bytes, _ := ioutil.ReadFile("./resources/maps/" + file)
	...
	for x := 0; x < 32; x++ {
		for z := 0; z < 32; z++ {
			...
			for {
				...
				for ; y < topColorStart; y++ { volume.data[x][y][z] = 0 }
				...
				if topColorEnd == volume.height-2 { volume.data[x][y][volume.height-1] = volume.data[x][y][volume.height-2] }
				...
				if bottomColorEnd == volume.height-1 { volume.data[x][y][volume.height-1] = volume.data[x][y][volume.height-2] }
			}
		}
	}
}

func (volume *volumeStruct) Load2(v* uint8, m* uint32) {
	bytes, _ := ioutil.ReadFile("./resources/maps/" + file)

	for y:=0; y< volume.depth; y++ {
		for x:=0; x<volume.width; x++ {
			z:= 0;
			for z; z<volume.height; z++ { volume.SetVoxel(x,z,y,RGBAToUint(192, 32,32,255)) }
			z= 0
			for {
				i                   :=0
				color               := nil     
				...
				for i=z; i<top_color_start; i++ {
					volume.SetVoxel(x,i,y,0)
				}

				color = uint32*(v+4)
				for z=top_color_start; z<=top_color_end; z++ { volume.SetVoxel(x,z,y,*color++) }
				...
				bottom_color_end   = v[3]
				bottom_color_start = bottom_color_end-len_top

				for z=bottom_color_start; z<bottom_color_end; z++ { volume.SetVoxel(x,z,y,*color++) }
			}
		}
	}
}
